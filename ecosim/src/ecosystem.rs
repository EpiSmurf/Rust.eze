/*
    ...
    This module defines the Ecosystem struct, which
    represents the simulation grid and contains the
    collections of plant, herbivore, and carnivore
    agents. It also implements the simulation step logic
    and statistics collection for each iteration.
*/
use crate::config::{
    SimulationConfig, Agent, AgentType
};
use rand::Rng;

#[allow(dead_code)]
/// Holds statistical data for a single
...
#[allow(dead_code)]
impl IterationStats {
    /// Prints the iteration statistics in a
    /// formatted output.
    pub fn print(
        &self,
...
#[derive(Clone)]
/// Represents the entire ecosystem, including grid
/// dimensions and agent collections.
...
impl Ecosystem {
    /// Creates a new ecosystem using a custom
        ...
        for _ in 0..config.initial_carnivores {
            ...
            carnivores.push(
                Agent::new(
                    ...
                    x,
                    y,
                    config.carnivore_initial_energy
                )
            );
            next_agent_id += 1;
        }
    ...
    /// Returns a list of indices for plants located at the
    ...
    /// Returns a list of indices for herbivores located at
    /// the specified (x, y) coordinates.
    fn find_herbivores_at(
        &self,
        x: usize,
        y: usize
    ) -> Vec<usize> {
    ...
    /// Returns a random adjacent coordinate (including
    /// the current cell) given a starting position.
    ...
    /// Advances the simulation by one step and returns the
    ...
    pub fn step(&mut self) -> IterationStats {
        ...
        for _plant in plants_copy {
            if rng.gen::<f32>() < self.config.plant_growth_rate {
                ...
                if let Some(existing_index) = self.plants
                    .iter()
                    .position(|p| {
                        p.x == nx &&
                        p.y == ny
                ...
                {
                    let new_type = match self.plants
                        [existing_index]
                        .agent_type
                    {
                        AgentType::Plant => {
                        ...
                        _ => continue,  // Should never
                                           // happen
                    };
                    self.plants[existing_index] = Agent::new(
                        self.plants[existing_index].id,
                        new_type,
                        nx,
                        ny,
                        0
        ...
        for mut herbivore in current_herbivores {
            ...
            herbivore.energy -=
                self.config.herbivore_energy_loss;
            let plant_indices = self.find_plants_at(
                herbivore.x,
                herbivore.y
            );
            if !plant_indices.is_empty() {
                ...
                herbivore.energy +=
                    self.config.herbivore_energy_gain;
                herbivores_eaten_count += 1;
            }
            if herbivore.energy >=
                self.config.herbivore_reproduction_threshold &&
                rng.gen::<f32>() <
                self.config.herbivore_reproduction_rate
            {
                let (ox, oy) = Self::random_adjacent_aux(
                    &mut rng,
                    herbivore.x,
                    herbivore.y,
                    self.width,
        ...
        let current_carnivores =
            std::mem::take(&mut self.carnivores);
        ...
        for mut carnivore in current_carnivores {
            ...
            carnivore.energy -=
                self.config.carnivore_energy_loss;
            let herbivore_indices = self.find_herbivores_at(
                carnivore.x,
                carnivore.y
            );
            if !herbivore_indices.is_empty() {
                ...
                carnivore.energy +=
                    self.config.carnivore_energy_gain;
                carnivores_eaten_count += 1;
            }
            if carnivore.energy >=
                self.config.carnivore_reproduction_threshold &&
                rng.gen::<f32>() <
                self.config.carnivore_reproduction_rate
            {
                let (ox, oy) = Self::random_adjacent_aux(
                    &mut rng,
                    carnivore.x,
        ...
        let (herb_energy_min, herb_energy_max,
            herb_energy_avg) = if !self.herbivores.is_empty() {
                let min = self.herbivores
                    .iter()
                    .map(|h| h.energy)
                    .min()
                    .unwrap();
                let max = self.herbivores
                    .iter()
                    .map(|h| h.energy)
                    .max()
                    .unwrap();
                let sum: i32 = self.herbivores
                    .iter()
                    .map(|h| h.energy)
                    .sum();
                let avg = sum as f32 /
                    self.herbivores.len() as f32;
                (Some(min), Some(max), Some(avg))
            } else {
                (None, None, None)
            };

        let (carn_energy_min, carn_energy_max,
            carn_energy_avg) = if !self.carnivores.is_empty() {
                let min = self.carnivores
                    .iter()
                    .map(|c| c.energy)
                    .min()
                    .unwrap();
                let max = self.carnivores
                    .iter()
                    .map(|c| c.energy)
                    .max()
                    .unwrap();
                let sum: i32 = self.carnivores
                    .iter()
                    .map(|c| c.energy)
                    .sum();
                let avg = sum as f32 /
                    self.carnivores.len() as f32;
                (Some(min), Some(max), Some(avg))
            } else {
                (None, None, None)
            };
    ...
    #[allow(dead_code)]
    /// Draws a simple text-based representation of the
    /// ecosystem grid to the console.
    pub fn draw(&self) {
        let mut grid = vec![
            vec!['.'; self.width]
        ][..self.height]
